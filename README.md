...
- **更可靠的启动与检索**：重复打开 Solaris 会唤醒已有窗口；全局搜索提供最近内容入口，并能准确定位任务、记录和文档；
- **更顺畅的收件箱与记录体验**：待确认事项处理后自动聚焦下一项，项目记录和事件记录新增标题大纲与快速定位；
- **成就海报**：可预览本期成就，保存为单页 A4 PDF，并在 Finder 中直接查看保存位置；
- **更清晰的项目推进**：项目概览集中展示下一步任务、检查项目、下一个节点、下一个事件和最近资源，并可从空态直接进入日期看板创建；
- **统一的日期与时间体验**：日期输入统一使用共享控件，并依据 macOS 系统时区和本地日期保持各页面时间判断一致；
- **更安全的本地备份恢复**：成就海报纳入备份，旧版本工作区会在隔离区完成迁移和校验后再恢复，大备份校验更加节省内存；
- **Bug 修复与体验优化**：集中修复并优化表单、日期看板、成就看板、文档中心、归档页面及常用浮层的布局、反馈、稳定性与性能。

完整变化请查看 [Solaris v1.2.5 发布说明](docs/releases/RELEASE_NOTES_v1.2.5.md)。
...
[前往最新版下载页面](https://github.com/EricGuo3/solaris-releases/releases/latest)，下载 `Solaris v1.2.5`：
...
```text
74d346636d06c853e604c1156be003ffb700ead3fc8a54b234ad3ddb8c203b76
```

下载后在终端执行：

```bash
shasum -a 256 Solaris_1.2.5_arm64.dmg
```
